...
.world-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:16px}.place-card{padding:18px;border-radius:18px;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.08);transition:transform .25s ease,border-color .25s ease,background .25s ease}.place-card:hover{transform:translateY(-2px);border-color:rgba(255,75,82,.28);background:rgba(255,255,255,.075)}
.place-head{display:flex;align-items:center;gap:11px}.place-head .flag{font-size:29px}.place-head b,.place-head small{display:block}.place-head small{color:var(--muted);margin-top:2px}.clock{font-size:clamp(38px,5vw,64px);font-weight:900;letter-spacing:-.06em;line-height:1;margin:22px 0 14px}.weather-line{display:flex;align-items:center;gap:9px}.weather-line>span:first-child{font-size:25px}.weather-line>b{font-size:23px}.weather-line>span:last-child{color:var(--muted);font-size:13px}.weather-meta{display:flex;gap:18px;margin-top:13px;color:var(--muted);font-size:12px}.weather-meta b{color:#fff}.weather-updated{color:var(--muted);font-size:11px}
.card,.stats article,.world-widget,.shopping-card,.workout,.task,.date-card,.note-card{box-shadow:0 16px 45px rgba(0,0,0,.18);backdrop-filter:blur(16px)}.primary,.nav,.plus,.link,.outline,.filter,.place-card{transition:all .22s ease}.primary:hover,.plus:hover{transform:translateY(-1px);box-shadow:0 8px 24px rgba(227,38,46,.25)}
dialog{max-height:94vh;overflow:auto}dialog form{max-height:90vh;overflow:auto}.wide{width:min(94vw,820px)}
...
async function getWeather(key){const p=places[key],url=`https://api.open-meteo.com/v1/forecast?latitude=${p.lat}&longitude=${p.lon}&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,is_day&timezone=${encodeURIComponent(p.tz)}`;const r=await fetch(url);if(!r.ok)throw new Error('Weather request failed');const j=await r.json(),c=j.current,[label,dayIcon]=weatherText[c.weather_code]||['Weather','🌡️'],icon=c.is_day?dayIcon:(c.weather_code<=2?'🌙':dayIcon);document.getElementById(`${key}Icon`).textContent=icon;document.getElementById(`${key}Temp`).textContent=`${Math.round(c.temperature_2m)}°`;document.getElementById(`${key}Weather`).textContent=label}
async function refreshWeather(){const status=document.getElementById('weatherUpdated');try{await Promise.all([getWeather('norway'),getWeather('cebu')]);status.textContent='Updated '+new Date().toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}catch(e){console.error(e);status.textContent='Weather unavailable'}}
updateClocks();setInterval(updateClocks,1000);refreshWeather();setInterval(refreshWeather,15*60*1000);
for(const [tag,attrs] of [
 ['script',{src:'public-sync.js?v=20260729-1'}],
 ['script',{src:'slideshow-v3.js?v=20260730-2'}],
 ['link',{rel:'stylesheet',href:'dashboard-v4.css?v=20260730-3'}],
 ['script',{src:'dashboard-v4.js?v=20260730-3'}],
 ['link',{rel:'stylesheet',href:'dashboard-v5.css?v=20260730-1'}],
 ['script',{src:'dashboard-v5.js?v=20260730-1'}],
 ['link',{rel:'stylesheet',href:'dashboard-v6.css?v=20260731-1'}],
 ['script',{src:'dashboard-v6.js?v=20260731-1'}]
]){const el=document.createElement(tag);Object.assign(el,attrs);(tag==='link'?document.head:document.body).appendChild(el)}
})();