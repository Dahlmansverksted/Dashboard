...
function drawRadar(){
 ...
 if(card){
  const small=card.querySelector('.head small'),title=card.querySelector('.head h3');
  if(small)small.textContent='TRAINING DISTRIBUTION';
  if(title)title.textContent='Muscle group balance';
 }
 ...
 c.strokeStyle='rgba(216,176,139,.13)';
 GROUPS.forEach((_,i)=>{const p=point(cx,cy,r,i,n);c.beginPath();c.moveTo(cx,cy);c.lineTo(...p);c.stroke()});
 const dataPts=values.map((v,i)=>point(cx,cy,r*(v/max),i,n));
 polygon(c,dataPts);c.fillStyle='rgba(216,176,139,.24)';c.fill();c.strokeStyle='#d8b08b';c.lineWidth=3;c.stroke();
 ...
 GROUPS.forEach((name,i)=>{
  const p=point(cx,cy,r+34,i,n),v=values[i];
  const a=-Math.PI/2+i*Math.PI*2/n;c.textAlign=Math.cos(a)>.25?'left':Math.cos(a)<-.25?'right':'center';
  c.fillText(`${name} ${v}`,p[0],p[1]);
 });
...
function applyFixes(){
 ...
 if(changed&&typeof save==='function')save('Wash face restored');
 else if(dailyList&&![...dailyList.querySelectorAll('span')].some(x=>x.textContent.trim().toLowerCase()==='wash face')&&typeof renderDaily==='function')renderDaily();
 drawRadar();
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(applyFixes,50));
else setTimeout(applyFixes,50);
